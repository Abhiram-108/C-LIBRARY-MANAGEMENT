...
Book* rightrotate1(Book* p){
    Book* c=p->left;
    Book* t=c->right;
    c->right=p;
    p->left=t;
    p->height=max(maxdepth1(p->left),maxdepth1(p->right))+1;
    c->height=max(maxdepth1(c->left),maxdepth1(c->right))+1;
return c;

}
Student* rightrotate2(Student* p){
    Student* c=p->left;
    Student* t=c->right;
    c->right=p;
    p->left=t;
    p->height=max(maxdepth2(p->left),maxdepth2(p->right))+1;
    c->height=max(maxdepth2(c->left),maxdepth2(c->right))+1;
return c;

}
Book* leftrotate1(Book* c){
    Book* p=c->left;
    Book* t=p->right;
    p->left=c;
    c->right=t;
...
Book* rotate(Book* root){
    if(maxdepth1(root->left)-maxdepth1(root->right)>1){
        if(maxdepth1(root->left->left)-maxdepth1(root->left->right)>0){
            //left-left -case
        ...
        if(maxdepth1(root->right->left)-maxdepth1(root->right->right)<0){
            //left-left -case
            return leftrotate1(root);
        }
        if(maxdepth1(root->right->left)-maxdepth1(root->right->right)<0){
            root->left=rightrotate1(root->left);
            return leftrotate1(root);
        }
    }
}
Student* rotate2(Student* root){
    if(maxdepth2(root->left)-maxdepth2(root->right)>1){
        if(maxdepth2(root->left->left)-maxdepth2(root->left->right)>0){
            //left-left -case
        ...
        if(maxdepth2(root->right->left)-maxdepth2(root->right->right)<0){
            //left-left -case
            return leftrotate2(root);
        }
        if(maxdepth2(root->right->left)-maxdepth2(root->right->right)<0){
            root->left=rightrotate2(root->left);
            return leftrotate2(root);
        }
    }
}
